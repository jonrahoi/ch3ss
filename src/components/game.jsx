...
const whiteMaterial = new THREE.MeshLambertMaterial({color: 0xd3d3d3});
...
const boardMaterial = new THREE.MeshBasicMaterial({color: 0xf5f5f5, transparent: true, opacity: 0.05, wireframe: true});
const boardGeometry = new THREE.BoxGeometry(1, 1, 1);
//const edges = new THREE.LineSegments(boardGeometry, boardMaterial);
...
let SPACE_SELECTION_MODE = true;

function getGamePositionFromObject(object) {
    let position_3D = object.position;
    let x = (position_3D.x + 3).toString();
    let y = (position_3D.y + 3).toString();
    let z = (position_3D.z + 3).toString();

    return currentGame.getPositionFromString(x + y + z);
}

function setValidSpaces(piece) {
    let validSpaces = [];
    let validPositions = currentGame.getPossibleMovesForPieceAtSpace(getGamePositionFromObject(SELECTED_PIECE));

    for (let i = 0; i < validPositions.length; i++) {
        let position = validPositions[i];
        for (let j = 0; j < board.children.length; j++) {
            let space = board.children[j];
            if (space.userData.x == position.getX() && space.userData.y == position.getY() && space.userData.z == position.getZ()) {
...
function highlightValidSpaces() {
    if (VALID_SPACES != null) {
        for (let i = 0; i < VALID_SPACES.length; i++) {
            ...
            space.material.color.setHex(0x000000);
        }
    }
}

function unhighlightValidSpaces() {
    for (let i = 0; i < VALID_SPACES.length; i++) {
        let space = VALID_SPACES[i];
        space.material.color.setHex(boardMaterial.color);
    }
}
...
export default class LiveGame extends Component {
    componentDidMount() {
        ...
        function onMouseDown( event ) {
            ...
            if (SPACE_SELECTION_MODE) {
                console.log("PIECE SELECTION MODE");
                ...
                if (aPieceWasSelected) {
                    ...
                    intersects[intersection].object.material.color.setHex(0xff0000);
